...
**M0 unblock → M1 playable demo → M2 quality/selection → M3 learning model → M4 dashboard → (M5 AI ∥ M6 hardening) → M7.**
...
- **Blockers:** `GROQ_API_KEY`, `SUPABASE_SERVICE_ROLE_KEY`. **Founder decision:** AI grounding/safety guardrails + per-user cost caps (before generation goes live).
...
3. **M5** — AI grounding/safety guardrails and per-user cost caps.

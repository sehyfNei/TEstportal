-- TSP-036 diagnostic weighted question selection.

create or replace function public.select_diagnostic_questions(
  p_exam_id uuid,
  p_count int,
...
as $$
  with ranked_topics as (
    select
      t.id as topic_id,
      coalesce(t.weight_percent, 0)::numeric as weight_percent,
      row_number() over (
        order by coalesce(t.weight_percent, 0)::numeric desc, t.order_index, t.id
      ) as weight_rank,
      count(*) over () as topic_count
    from public.topics t
    where t.exam_id = p_exam_id
      and t.level = 1
      and coalesce(t.weight_percent, 0)::numeric > 0
  ),
  base_allocs as (
    select
      rt.topic_id,
      rt.weight_rank,
      rt.topic_count,
      floor(greatest(coalesce(p_count, 0), 0) * rt.weight_percent / 100)::int as base_alloc
    from ranked_topics rt
  ),
  alloc_summary as (
    select
      coalesce(sum(base_alloc), 0)::int as allocated,
      coalesce(max(topic_count), 0)::int as topic_count
    from base_allocs
  ),
  extra_allocs as (
    select
      ((slot - 1) % alloc_summary.topic_count) + 1 as weight_rank,
      count(*)::int as extra_alloc
    from alloc_summary
    cross join generate_series(
      1,
      greatest(greatest(coalesce(p_count, 0), 0) - alloc_summary.allocated, 0)
    ) as slot
    where alloc_summary.topic_count > 0
    group by ((slot - 1) % alloc_summary.topic_count) + 1
  ),
  topic_allocs as (
    select
      ba.topic_id,
      ba.base_alloc + coalesce(ea.extra_alloc, 0) as alloc
    from base_allocs ba
    left join extra_allocs ea on ea.weight_rank = ba.weight_rank
    where ba.base_alloc + coalesce(ea.extra_alloc, 0) > 0
  ),
  weighted_picks as (
    select
      q.id as question_id,
    ...
    join topic_allocs ta on ta.topic_id = q.topic_id
    where q.exam_id = p_exam_id
      and q.status in ('approved', 'live')
      and q.exposure_policy = any(coalesce(p_exposure_policies, array['practice']))
  ...
  selected as (
    select
      wp.question_id,
      wp.current_version_id,
      wp.q_type,
      wp.content
    from weighted_picks wp
    join topic_allocs ta on ta.topic_id = wp.topic_id
    where wp.rn <= ta.alloc
  ),
  fillup as (
    select
      q.id as question_id,
    ...
    from public.questions q
    join public.question_versions qv on qv.id = q.current_version_id
    where exists (select 1 from topic_allocs)
    ...
    order by random()
    limit greatest(0, greatest(coalesce(p_count, 0), 0) - (select count(*) from selected))
  )
  select *
  from selected
  union all
...
revoke all on function public.select_diagnostic_questions(uuid, int, text, text[]) from public;

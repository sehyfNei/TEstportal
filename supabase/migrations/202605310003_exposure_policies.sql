-- TSP-030 exposure policies: admin assignment plus session-type pool selection.

create or replace function public.set_question_exposure_policy(
  p_question_id uuid,
  p_policy text
)
returns jsonb
...
begin
  ...
  if p_policy not in ('practice','diagnostic_reserved','benchmark_reserved','hidden') then
    raise exception 'invalid exposure policy: %', p_policy using errcode = '22023';
  end if;

  select exposure_policy
  into v_old_policy
  ...
  return jsonb_build_object(
    ...
    'new_policy', p_policy,
    'changed', v_old_policy is distinct from p_policy
  );
end;
$$;
...
revoke all on function public.set_question_exposure_policy(uuid, text) from public;
revoke all on function public.start_test_session(uuid, text, uuid, uuid, int, int, text) from public;

grant execute on function public.set_question_exposure_policy(uuid, text) to authenticated;

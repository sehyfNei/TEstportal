-- Previous-year-only topic practice (TSP-194).
-- Adds an optional p_pyq_only filter to topic practice so students can drill
-- only previous-year questions (questions.source = 'pyq'). Threads a matching
-- p_pyq_only param through start_test_session, applied ONLY to the 'topic'
-- branch. Both params default false, so every existing caller is unchanged.
-- The old 7-arg start_test_session is dropped to avoid an overload ambiguity
-- for PostgREST once the 8-arg version exists.
-- Rollback: re-apply select_topic_practice_questions + start_test_session from
-- 202606030001_concept_retest_routing.sql (and 202606010001) and
-- drop function start_test_session(uuid,text,uuid,uuid,int,int,text,boolean).
...
as $$
  ...
  eligible as (
    ...
    where q.exam_id = p_exam_id
      ...
      and (not p_pyq_only or q.source = 'pyq')
  ...
  easy_picks as (
    ...
    order by random()
    limit greatest(0, floor(greatest(coalesce(p_count, 0), 0) * 0.3)::int)
  ),
  medium_picks as (
    select question_id, current_version_id, q_type, content
    from eligible
    ...
    limit greatest(
      ...
      greatest(coalesce(p_count, 0), 0)
        - 2 * floor(greatest(coalesce(p_count, 0), 0) * 0.3)::int
    )
  ),
  hard_picks as (
    select question_id, current_version_id, q_type, content
  ...
  fillup as (
    ...
    where not exists (
      select 1
      from balanced b
      where b.question_id = eligible.question_id
    )
    order by random()
    limit greatest(
      0,
      greatest(coalesce(p_count, 0), 0) - (select count(*) from balanced)::int
    )
  )
...
-- Drop the old 7-arg signature so only the 8-arg version below remains.
